...
def view_votes(request):
    ...
    candidate_votes = VotingResult.objects.values('candidate').annotate(vote_count=Count('candidate')).order_by(
        '-vote_count')

    # Find the candidate with the most votes
    most_voted_candidate = None
    if candidate_votes:
        most_voted_candidate = Candidate.objects.get(id=candidate_votes[0]['candidate'])
